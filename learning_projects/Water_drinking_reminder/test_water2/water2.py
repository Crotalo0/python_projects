...
def acqua_max():
	...
	if fa_caldo == "s" :
		...
		return	
	my_file.write(f"Peso: {peso} kg, Workout: {workout} minuti  Capienza bicchiere: {bicchiere} ml, Caldo: {fa_caldo} \n\n")	
...
def ho_bevuto():
	...
	my_file.write(f"Bicchiere n.{counter}, Ora: {local_time}, Acqua mancante: {acqua_rimanente}\n")
...
label_1=tk.Label(master=frame, text="Peso (in kg): ", height=1)
label_2=tk.Label(master=frame, text="Minuti di allenamento: ", height=1)
label_3=tk.Label(master=frame, text="Capienza bicchiere(in ml): ", height=1)
label_4=tk.Label(master=frame, text="Fa molto caldo?: ", height=1)
label_5=tk.Label(master=frame3, text="", height=1)
label_6=tk.Label(master=frame3, text="0", height=1)
label_7=tk.Label(master=frame3, text="Bicchieri bevuti: ", height=1)

lbl_result0 = tk.Label(master=frame2, text="Acqua da bere totale (ml): ")
lbl_result = tk.Label(master=frame2, text="-----")
lbl_result1 = tk.Label(master=frame2, text="Acqua mancante (ml): ")
lbl_result2 = tk.Label(master=frame2, text="-----")
lbl_result3 = tk.Label(master=frame2)

entry_1=tk.Entry(master=frame, width=6)
entry_2=tk.Entry(master=frame, width=6)
entry_3=tk.Entry(master=frame, width=6)
entry_4=tk.Entry(master=frame, width=6)

chkValue = tk.BooleanVar() 
chkValue.set(False)

btn_si = tk.Checkbutton(master = frame, var=chkValue, command=opasd)
entry_4.insert(0,"n")

btn_1 = tk.Button(master = frame1, text="Calcola acqua da bere", command=acqua_max)
btn_2 = tk.Button(master = frame1, text="Ho Bevuto!", command=ho_bevuto)
btn_3 = tk.Button(master = frame1, text="mostra log", command=openNewWindow)
