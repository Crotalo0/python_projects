#Progetto 6
#Contact book, ovvero una rubrica
...
def main():
    ...
    #Creo un cursore e lo lego a conn
    cur = conn.cursor()
    #Creo una tabella, con determinati parametri.
    cur.execute("""
        ...
        gender TEXT);
""")
    conn.commit() #Applico i cambiamenti al database.
...
""")
    ...
    #Inserisco 'o ignoro' in users i valori.
    cur.execute("""INSERT OR IGNORE INTO users(userid, fname, lname, gender) 
   VALUES('00001', 'Nik', 'Piepenbreier', 'male');""")
    conn.commit()
    
    #Posso inserire dei valori anche con delle tuple.
    user = ('00002', 'Lois', 'Lane', 'Female')
    #Per inserire le tuple devo usare ? e poi il nome della lista dopo una virgola.
    ...
    #Per inserire piu tuple contemporaneamente, usare executemany. 
    #ATTENZIONE: i valori devono essere tuple. ex. [(valori),(valori)]
    cur.executemany("INSERT OR IGNORE INTO users VALUES(?, ?, ?, ?);", customers)
    cur.executemany("INSERT OR IGNORE INTO orders VALUES(?, ?, ?, ?);", orders)
    conn.commit()

    #In questo modo chiamo tutti le tuple salvate in users, con fetchone() ne chiamo solo 1. A quanto pare il primo.
    cur.execute("SELECT * FROM users;")
    one_result = cur.fetchone()
    print(one_result)

    #Come su ma .fetchmany(n) con quante entry voglio leggere, le prime n insomma.
    cur.execute("SELECT * FROM users;")
    three_results = cur.fetchmany(3)
    print(three_results)

    #.fetchall() legge tutto quanto.
    cur.execute("SELECT * FROM users;")
    all_results = cur.fetchall()
    print(all_results)

    #Leggo tutto da orders.
    cur.execute("SELECT * FROM orders;")
    all_results = cur.fetchall()
    print(all_results)

    #Elimino un entry di users che ha un elemento che decido io.
    #In questo caso elimino lname= 'Parker'
    cur.execute("DELETE FROM users WHERE lname='Parker';")
    conn.commit()
    #Seleziono da users l'elemento eliminato in precedenza per verificare che sia stato davvero eliminato.
    cur.execute("select * from users where lname='Parker'")
    print(cur.fetchall())


    #Qui non ho ben capito purtroppo.
